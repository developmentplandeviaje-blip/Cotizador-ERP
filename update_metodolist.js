...
// 3. Imprimir to Asignar
const imprimirBtnRegex = /<button[\s\S]*?className="btn-secondary"[\s\S]*?onClick=\{\(\) => window\.print\(\)\}[\s\S]*?title="Imprimir listado"[\s\S]*?>[\s\S]*?<img src=\{imgImprimir\}[\s\S]*?<\/button>\s*<span className='title-input'>Imprimir<\/span>/;
const asignarBtn = `<button
...
code = code.replace(imprimirBtnRegex, asignarBtn);
...
console.log("MetodoPagoList updated.");
