...
class UpdateMetodoPagoRequest extends FormRequest
{
    ...
    {
        $tipo = $this->input('tipo', $this->route('metodoPago')?->tipo);

        $rules = [
            'nombre' => ['sometimes', 'required', 'string', 'max:100'],
            'nombre_publico' => ['sometimes', 'required', 'string', 'max:200'],
            'tipo' => ['sometimes', 'required', Rule::in(['banco', 'digital', 'efectivo'])],
        ...
        if ($tipo === 'banco') {
            $rules['titular'] = ['sometimes', 'required', 'string', 'max:200'];
            $rules['tipo_documento'] = ['sometimes', 'required', 'string', 'max:10'];
            $rules['documento'] = ['sometimes', 'required', 'string', 'max:20'];
            ...
            $rules['correo_cuenta'] = ['sometimes', 'required', 'email', 'max:200'];
